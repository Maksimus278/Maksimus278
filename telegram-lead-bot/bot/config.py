...
def _resolve_path(raw_value: str | None, default_rel: str) -> Path:
    value = (raw_value or "").strip()
    candidate = Path(value) if value else ROOT / default_rel
...
LEADS_CSV_PATH = _resolve_path(os.getenv("LEADS_CSV_PATH"), "data/fleetguard-leads.csv")
if not LEADS_CSV_PATH.exists():
    sample = ROOT / "data" / "fleetguard-leads.sample.csv"
    if sample.exists():
        LEADS_CSV_PATH = sample

LEADS_DB_PATH = _resolve_path(os.getenv("LEADS_DB_PATH"), "data/leads_state.db")
